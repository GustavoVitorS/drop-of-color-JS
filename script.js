...
const ALPHA = 0.006;
const COLORS = [
  `hsla(130, 200%, 90%, ${ALPHA})`,
  `hsla(110, 120%, 30%, ${ALPHA})`,
  `hsla(640, 200%, 50%, ${ALPHA})`,
  `hsla(0, 100%, 50%, ${ALPHA})`,
];
...
const generate = () => {
  ...
  const polies = new Array(NUM_SPOTS).fill().map((_, i) => {
    const poly = getPoly(
      Math.cos(ANGLE_STEP * i) * (R * 1.5) * randomGaussian(1,),
      Math.sin(ANGLE_STEP * i) * (R * 1.5) * randomGaussian(1,),
      R + randomBetween(-25, 25),
      EDGES
    );

    return deformPoly(poly, DEPTH, VARIANCE_DEFAULT, 6);
  });

  ctx.fillStyle = stitchPattern;
  ctx.fillRect(1, 1, ctx.canvas.width, ctx.canvas.height);
